  ...
  return <>
    ...
    <div className="px-8 py-4 md:ml-96 flex flex-col">
      ...
      <main>
        ...
        <div className="h-6" />
        <p>In probability theory, we define a concept called &quot;random variable&quot;, usually denoted by a capital, say <InlineMath math='X' />.  <InlineMath math='X' /> is a random quantity that takes a different value each time it is evaluated. Assume <InlineMath math='X' /> is continuous, the probability that <InlineMath math='X' /> takes a specific value <InlineMath math='x' /> is given by the probability density function (PDF), denoted by  <InlineMath math='f(x)' />. In other words, <InlineMath math='f(x) = P(X = x)' />. The PDF can take any form, as long as the integral across the its domain, or the support <InlineMath math='S' /> of the random variable, is 1, that is <InlineMath math='\int_{x\in S} f(x) dx = 1' />. The PDF uniquely identifies a random variable.
          <div className="h-3" />

          There are some PDFs that arises frequently in science, either naturally or artificially. The most famous one is probably <InlineMath math='f(x) = \frac{1}{\sigma\sqrt{2\pi}} e^{-\frac{1}{2}(\frac{x - \mu}{\sigma})}' />. Any random variable with a PDF of this form is said to have a normal distribution of <InlineMath math='\mathcal{N}(\mu, \sigma)' />. Other popular probability distributions include chi-squared distribution, poisson distribution, weibull distribution, etc. Their importance is embodied many fields of science (even outside of maths or statistics), including physics, biology, sociology, and so on.
        </p>
        <div className="h-12" />
